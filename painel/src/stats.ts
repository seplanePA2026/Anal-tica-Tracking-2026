...
export function colorFor(label: string): string {
  const known: Record<string, string> = {
    'Lula (PT)': '#9c2f2f',
    'Flávio Bolsonaro (PL)': '#1e3d59',
    'Jerônimo Rodrigues (PT)': '#9c2f2f',
    'Jerônimo Rodrigues': '#9c2f2f',
    'ACM Neto (UNIÃO BRASIL)': '#1e3d59',
    'ACM Neto': '#1e3d59',
    'ACM Neto (União Brasil': '#1e3d59',
    Aprova: '#2c6e49',
    Desaprova: '#9c2f2f',
    Ótimo: '#2c6e49',
    Bom: '#4d8f6a',
    'Regular mais para positivo': '#8aa36b',
    'Regular mais para negativo': '#c48a4a',
    Ruim: '#b45c3a',
    Péssimo: '#7a2e2e',
    Feminino: '#6b4c7a',
    Masculino: '#1e3d59',
    Definitiva: '#2c6e49',
    'Pode mudar': '#c48a4a',
    'Conhece e vota': '#2c6e49',
    'Conhece e não vota': '#c48a4a',
    'Não o conhece': '#7a746c',
    'Aliada de Lula': '#9c2f2f',
    'Aliada de Flávio Bolsonaro': '#1e3d59',
    Independente: '#5c6b73',
    Lulista: '#9c2f2f',
    Bolsonarista: '#1e3d59',
  }
  if (known[label]) return known[label]
  if (label === EMPTY) return '#b7b1a6'
  let h = 0
  for (let i = 0; i < label.length; i++) h = (h * 33 + label.charCodeAt(i)) >>> 0
  const palette = ['#4a5d73', '#7a5c4e', '#4f6f62', '#6d5a7a', '#7a6a3a', '#3f5f6e']
